...
class SearchControllers: UITableViewController {
...
extension SearchControllers: UISearchResultsUpdating {
    func updateSearchResults(for searchController: UISearchController) {
        // print("DEBUG: Update user list here")
        guard let searchText = searchController.searchBar.text?.lowercased() else { return }
        
        filterUsers = users.filter({ $0.username.lowercased().contains(searchText) || $0.fullname.lowercased().contains(searchText) })
        
        self.tableView.reloadData() // fullname username 안됨!!!
    }
}
